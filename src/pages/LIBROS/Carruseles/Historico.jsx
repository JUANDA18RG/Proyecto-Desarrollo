...
function Historico() {
  ...
  const [historicoBooks, setHistoricoBooks] = useState([]);
  ...
  const goToNextSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide((prev) => (prev + 1) % historicoBooks.length);
    }
  };
  ...
  const goToPreviousSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide(
        (prev) => (prev - 1 + historicoBooks.length) % historicoBooks.length
      );
    }
  };
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const librosHistoricos = response.data.filter(
          (libro) => libro.genero === "histórico"
        );
        setHistoricoBooks(librosHistoricos);
      })
      .catch((error) => {
  ...
  return (
    <div className="flex justify-center items-center mt-10">
      ...
      <div className="container mb-10">
        <h4 className="text-3xl font-bold mb-4">Categoría de Historico</h4>
        {loading ? (
          <p>Cargando libros...</p>
        ) : (
          <Carousel
          ...
          >
            {historicoBooks.map((libro, index) => (
              <div
                key={libro.ISBN}
