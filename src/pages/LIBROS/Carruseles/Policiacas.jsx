...
function Policiacas() {
  ...
  const [PoliciacasBooks, setPoliciacasBooks] = useState([]);
  ...
  const goToNextSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide((prev) => (prev + 1) % PoliciacasBooks.length);
    }
  };
  ...
  const goToPreviousSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide(
        (prev) => (prev - 1 + PoliciacasBooks.length) % PoliciacasBooks.length
      );
    }
  };
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const PoliciacasBooks = response.data.filter(
          (libro) => libro.genero === "policiacas"
        );
        setPoliciacasBooks(PoliciacasBooks);
      })
      .catch((error) => {
  ...
  return (
    <div className="flex justify-center items-center mt-10">
      ...
      <div className="container mb-10">
        <h4 className="text-3xl font-bold mb-4">Categoría Policiacas</h4>
        {loading ? (
          ...
          >
            {PoliciacasBooks.map((libro, index) => (
              <div
                key={libro.ISBN}
