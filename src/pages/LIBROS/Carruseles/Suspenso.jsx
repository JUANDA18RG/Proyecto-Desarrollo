...
function Suspenso() {
  ...
  const librosDeSuspenso = LibrosData.filter((libro) => libro.categoria === 'suspenso');

  const goToNextSlide = () => {
    if (carouselRef.current) {
      carouselRef.current.next(1);
      setCurrentSlide((prev) => (prev + 1) % librosDeSuspenso.length); // Hace que sea infinito
    }
  };
  ...
  const goToPreviousSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide((prev) => (prev - 1 + librosDeSuspenso.length) % librosDeSuspenso.length); // Hace que sea infinito
    }
  };
  ...
  return (
    ...
    <div className="container mb-10">
      ...
      <Carousel
        ...
        >
          {librosDeSuspenso.map((libro, index) => (
                       <div
                       key={libro.id}
