...
function Suspenso() {
  ...
  const [suspensoBooks, setsuspensoBooks] = useState([]);
  ...
  const goToNextSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide((prev) => (prev + 1) % suspensoBooks.length);
    }
  };
  ...
  const goToPreviousSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide(
        (prev) => (prev - 1 + suspensoBooks.length) % suspensoBooks.length
      );
    }
  };
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const librosSuspenso = response.data.filter(
          (libro) => libro.genero === "suspenso"
        );
        setsuspensoBooks(librosSuspenso);
      })
      .catch((error) => {
