...
function Accion() {
  ...
  const librosDeAccion = LibrosData.filter((libro) => libro.categoria === 'accion');

  const goToNextSlide = () => {
    if (carouselRef.current) {
      carouselRef.current.next(1);
      setCurrentSlide((prev) => (prev + 1) % librosDeAccion.length);
    }
  };
  ...
  const goToPreviousSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide((prev) => (prev - 1 + librosDeAccion.length) % librosDeAccion.length);
    }
  };
  ...
  return (
    <div className="flex justify-center items-center mt-10">
      ...
      <div className="container mb-10">
        <h4 className="text-3xl font-bold mb-4">Categoría de Acción</h4>
        <Carousel
        ...
        >
          {librosDeAccion.map((libro, index) => (
   <div
   key={libro.id}
