...
function Accion() {
  ...
  const [AccionBooks, setAccionBooks] = useState([]);
  ...
  const goToNextSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide((prev) => (prev + 1) % AccionBooks.length);
    }
  };
  ...
  const goToPreviousSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide(
        (prev) => (prev - 1 + AccionBooks.length) % AccionBooks.length
      );
    }
  };
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        const AccionBooks = response.data.filter(
          (libro) => libro.genero === "accion"
        );
        setAccionBooks(AccionBooks);
      })
      .catch((error) => {
  ...
  return (
    <div className="flex justify-center items-center mt-10 no-repeat bg-cover bg-center outline-none">
      ...
      <div className="container mb-10">
        <h4 className="text-3xl font-bold mb-4 bg-white bg-opacity-70 rounded-lg p-4 text-black inline-block  mt-4 relative z-10">
          Categoría de Accion
        </h4>
        {loading ? (
