...
function Drama() {
  ...
  const librosDeDrama = LibrosData.filter((libro) => libro.categoria === 'drama');

  const goToNextSlide = () => {
    if (carouselRef.current) {
      carouselRef.current.next(1);
      setCurrentSlide((prev) => (prev + 1) % librosDeDrama.length); // Hace que sea infinito
    }
  };
  ...
  const goToPreviousSlide = () => {
    if (carouselRef.current) {
      ...
      setCurrentSlide((prev) => (prev - 1 + librosDeDrama.length) % librosDeDrama.length); // Hace que sea infinito
    }
  };
